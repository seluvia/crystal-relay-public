...
function validatePayload(payload) {
  ...
  const diagnostics = normalize(payload.diagnostics);
  ...
  if (diagnostics.length > MAX_DIAGNOSTICS_LENGTH) {
    return { ok: false, message: "Sanitized logs are too large." };
  }
...
function buildGitHubIssue(payload) {
  ...
  const diagnostics = sanitize(normalize(payload.diagnostics));
